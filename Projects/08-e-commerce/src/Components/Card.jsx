import { useContext } from "react"
import { HiPlus, HiCheck } from "react-icons/hi2"
import { Global } from "../Utils/Contexts/Global"

const Card = ({ data }) => {
  const { 
    openDetail, closeDetail, setProductToShow, 
    cartProducts, setCartProducts,
    openCheckOut, closeCheckOut,
  } = useContext(Global)

  const showProduct = (product) => {
    closeCheckOut()
    setProductToShow(product)
    openDetail()
  }

  const addProducts = (product) => {
    closeDetail()
    setCartProducts([...cartProducts, product])
    openCheckOut()
  }

  const renderIcon = (id) => {
    const isInCart = cartProducts.filter(product => product.id === id).length != 0
    
    if (isInCart) {
      return (
        <button 
          className="
          absolute top-0 right-0 flex justify-center items-center bg-black w-6 h-6 rounded-full m-2 p-1
        ">
          <HiCheck className="h-6 w-6 text-green-700"></HiCheck>
        </button>
      )
    } else {
      return (
        <button 
          onClick={(event) => {
            event.stopPropagation()
            addProducts(data)
          }}
          className="
          absolute top-0 right-0 flex justify-center items-center bg-white w-6 h-6 rounded-full m-2 p-1
        ">
          <HiPlus className="h-6 w-6 text-black"></HiPlus>
        </button>
      )
    }
  }

  return (
    <div 
      onClick={(event) => {
        event.stopPropagation()  
        showProduct(data)
      }}
      className="
      bg-white cursor-pointer w-56 h-60 rounded-lg
    ">
      <figure className="
        relative mb-2 w-full h-4/5 
      ">
        <span className="
          absolute bottom-0 left-0 bg-white/60 rounded-lg text-black text-xs m-2 px-3 py-0.5
        "> {data.category} </span>
        <img className="
          w-full h-full object-cover rounded-lg
        " src={data.image} alt="" />

        { renderIcon(data.id) }

      </figure>
      <p className="
        flex justify-between text-black mx-2 gap-4
      ">
        <span className="
          text-sm font-light truncate 
        ">{data.title}</span>
        <span className="
          text-lg font-medium
        ">${data.price}</span>
      </p>
    </div>
  )
}

export { Card }